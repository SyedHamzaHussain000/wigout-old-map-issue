...
const BackgroundManager = () => {
  ...
  // Keep latest userData reference without breaking the IAP Listener subscription loop
  const userDataRef = useRef(userData);
  useEffect(() => {
    userDataRef.current = userData;
  }, [userData]);

  // ── 1. App Boot/Initial Sync: Fetch Active Status From Live DB ───────────
  useEffect(() => {
    const syncSubscriptionState = async () => {
      if (token) {
        try {
          ...
          console.log('[IAP Sync Error] Boot validation failed:', error?.message);
        }
      }
    };
  ...
  // ── 4. Fixed Global IAP Observers Framework ────────────────────────────────
  useEffect(() => {
    if (!token) return; // Listeners only active when user profile token is present
      ...
      const unsubscribeNotifeeForeground = notifee.onForegroundEvent(
        async ({ type, detail }) => {
          if (type === EventType.PRESS) {
            ...
            if (!placeDetails && placeId) placeDetails = { placeId };
...
//   Purchase,
// } from 'react-native-iap';
// import { verifyIAPReceipt } from './src/GlobalFunctions/main';
// import { UpdateProfile, updateNotificationSettings } from './src/redux/Slices';
// import { ShowToast } from './src/utils/api_content';
...
//               // Derive subType from productId
//               const subType = planId.includes('monthly') ? 'monthly' : 'weekly';
//               const plan = planId.includes('premium')
//                 ? 'premium'
//                 : 'basic';

//               // Backend verification via new IAP endpoint
//               const res = await verifyIAPReceipt(token, {
//                 platform: Platform.OS === 'android' ? 'google' : 'apple',
//                 subType,
//                 plan,
...
//               if (res?.success) {
//                 ShowToast('success', 'Subscription processed successfully.');
//                 dispatch(UpdateProfile(res?.user ? { ...userData, subscription: res?.subscription } : res?.data));
//               } else if (token) {
