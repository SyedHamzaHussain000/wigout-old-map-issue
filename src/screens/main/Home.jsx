...
const Home = () => {
  ...
  // Strict API Lock Control Refs to block parallel duplicate calls
  const isFetchingNearby = useRef(false);
  const isFetchingRecommended = useRef(false);
  ...
  // Precision check to block unnecessary re-renders on floating numbers
  const locationKey = useMemo(() => {
    if (currentLocation?.latitude && currentLocation?.longitude) {
      return `${Number(currentLocation.latitude).toFixed(4)},${Number(
  ...
  // Local Client side filter implementation for recommendations
  const displayedRecommended = useMemo(() => {
    ...
    return recommendedLocations.filter(item => {
      ...
      if (catType && itemTypes.includes(catType)) return true;
      if (
        catName &&
        itemCategory &&
  ...
  const displayedNearby = useMemo(() => {
    if (!selectedCategory) return fetchedLocations;
  ...
  // API Call With Lock Gates to guarantee 1 single API instance execution
  const fetchData = useCallback(
    async (showLoader = true) => {
      if (isFetchingNearby.current) return;
      isFetchingNearby.current = true;
      ...
      try {
        ...
        isFetchingNearby.current = false;
      }
  ...
  const fetchRecommendedData = useCallback(async () => {
    if (isFetchingRecommended.current) return;
    ...
    try {
      ...
      dispatch(setRecommendedPlaces(broadResults || []));
    } catch (error) {
      console.log('Error updating recommendations pool:', error);
    } finally {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      console.error('Error handling notifications sync:', error);
    }
  }, [token]);
  ...
  const onRefresh = async () => {
    ...
    if (token) {
      try {
        ...
        console.log('Refresh sequencing optimization dropped:', error);
      }
    }
  ...
  }, [locationKey, selectedCategory?.id, isFocussed]);

  useEffect(() => {
    if (token && isFocussed) {
      fetchRecommendedData();
    }
  }, [token, locationKey, isFocussed]);

  useEffect(() => {
    const refreshAllData = async () => {
      if (!token || !isFocussed) return;
      try {
        const [revRes, wishRes] = await Promise.all([
          GetReviews(token),
          GetWishList(token),
        ...
        console.log('Error initializing runtime sync:', error);
      }
    };
  ...
  const renderRecommendedItem = useCallback(
    ...
    [getDisplayCategory, navigateToRoute],
  );
  ...
  const renderNearbyItem = useCallback(
    ...
    [selectedCategory?.name, getDisplayCategory, nearbyAnim, navigateToRoute],
  );
            ...
            <Animated.View
              ...
              <FlatList
                ...
                keyExtractor={(item, index) =>
                  `rec-item-${
                    item.place_id || item.placeId || item?._id || index
                  }`
                }
    ...
    <ScreenWrapper>
      {isLoading ? (
        ...
        <FlatList
          ...
          numColumns={2}
          keyExtractor={(item, index) =>
            `nearby-item-${item.place_id || item.placeId || item?._id || index}`
          }
...
const styles = StyleSheet.create({
  ...
  locationText: {maxWidth: responsiveWidth(40)},
  profileImage: {
    width: 50,
    height: 50,
    borderRadius: 25,
  ...
  notificationIcon: {width: 25, height: 25, tintColor: '#FFF'},
  sectionHeader: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  tabScrollContent: {paddingHorizontal: responsiveWidth(5), gap: 10},
  tabItem: {
  ...
  activeTabItem: {backgroundColor: '#47082E', borderColor: '#47082E'},
